...
exports.getAllStories = async (req, res) => {
  const story = await Stories.find().exec();

  res.status(200).json({ story });
};

////////// view count plus after seing//////////////
...
exports.getviewcount = async (req, res) => {
  ...
  const story = await Stories.findOneAndUpdate(
    //{ _id : { $nin: [user_id] }},
    { _id: story_id }, // here surched id if find then below update
    { $inc: { viewcount: 1 }, $addToSet: { viewedBy: [user_id] } },
    { new: true }
  ) // for addToSet --> the user_id will be inserted in an
...
// below code checking after seeing count 1+ only
